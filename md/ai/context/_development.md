...
- New CLI commands, flags, or behavior should be verified against the in-browser CLI runtime (`workspaces/libraries/web-common/web-cli-runtime`, consumed by both `workspaces/web/web-cli` and documentation-website's `/web-cli` page) before considering the change done. That runtime shims Node APIs (`fs`, `os`, `child_process`, etc.) narrowly to what pacwich currently touches, so a feature that exercises a new one can break the browser build. This isn't just a nice-to-have: `bun docs build` compiles `/web-cli`'s module graph as part of documentation-website's site-wide build (even though the page itself is client-only), so a real break there fails the whole doc site build and can block deployment. If a new feature needs a Node API not yet shimmed, extend `web-cli-runtime/stubs.ts`, or gate the feature in `webCliGuards.ts` if it can't reasonably work in-browser (e.g. real filesystem/git access).
