...
the [getting started guide](/intro/getting-started) to jump straight in.
...
You can orchestrate [running scripts](/cli/commands#run-script) from workspaces' package.json files.

The optional configuration enriches workspaces with additional metadata and can
define [rules](/config/workspace) for enforcing code sharing restrictions between workspaces.
...
`pacwich` includes a [`verify` feature](/concepts/verify) to help detect missing dependencies via simple code analysis.
