...
* <span className="deprecated"><b>Fix</b> (possibly breaking)</span>: The `run` and `run-affected` `-d` option (short for `--dep-order`) clashed with the global option `-d` (short for `--cwd`). The `--dep-order` short form is now `-D`.
