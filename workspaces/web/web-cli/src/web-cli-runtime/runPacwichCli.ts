...
const runTokens = async (
  ...
  // The CLI resolves the project from `--cwd`; `--pm pnpm` makes discovery
  // deterministic (the mock repo only carries a pnpm-lock.yaml anyway).
  // `--disable-executable-configs` actually enforces what stubs.ts's jiti
  // stub already assumes: config `.ts`/`.js` files are never evaluated.
  const argv = [
    "--cwd",
    PROJECT_ROOT,
    "--pm",
