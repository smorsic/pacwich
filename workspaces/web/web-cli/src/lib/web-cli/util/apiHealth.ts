      ...
      try {
        ...
        healthResponse
          .then((response) => {
            ...
            if (!newIsHealthy) {
              newError = new Error("Web CLI is not healthy");
              // eslint-disable-next-line no-console
              console.error("Web CLI is not healthy", response);
              setIsReady(false);
            }
          })
          .catch((error) => {
            // eslint-disable-next-line no-console
            console.error("Error loading web CLI health", error);
        ...
        readyResponse
          .then((response) => {
            if (!newError && !response.isReady) {
              newError = new Error("Web CLI is not ready");
              // eslint-disable-next-line no-console
              console.error("Web CLI is not ready", response);
            } else {
              setIsReady(true);
            }
          })
          .catch((error) => {
            // eslint-disable-next-line no-console
            console.error("Error loading web CLI ready state", error);
            newError = error as Error;
          });
        ...
        // eslint-disable-next-line no-console
        console.error("Error initializing web CLI", error);
