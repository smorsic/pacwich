/**
 * Adapted from documentation-website's terminal/exampleCommands.ts. The old
 * list targeted a demo project with `fe`/`be`/`shr` aliases and an
 * "application" tag; this workspace's demo project (`src/demo-project/`,
 * @demo/utils <- @demo/core <- @demo/web) doesn't define aliases or tags yet,
 * so alias-/tag-specific examples are dropped in favor of name/path/wildcard
 * patterns the demo project actually supports.
 */
...
export const EXAMPLE_COMMANDS = [
  ...
  },
  {
    ...
    description:
      'The workspaces that depend on the "@demo/utils" workspace will wait until its script is complete',
  ...
  },
  {
    ...
    command: 'run --workspace-patterns="@demo/core @demo/web" --script=build',
  },
  {
