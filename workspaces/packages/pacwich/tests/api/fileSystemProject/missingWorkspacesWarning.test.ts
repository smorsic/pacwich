...
describe("createFileSystemProject — missing-workspaces warning", () => {
  ...
  const findHintCall = (hintSubstring: string) =>
    warnSpy.mock.calls.find(
      ([id, options]: [string, { hint?: string }]) =>
        id === "MissingWorkspacesHint" &&
        typeof options?.hint === "string" &&
        options.hint.includes(hintSubstring),
    );
  ...
  test("bun: warns when package.json has no workspaces field", () => {
    ...
    expect(findHintCall("No workspaces declared")).toBeDefined();
  });
  ...
  test("bun: does NOT warn when package.json has an (empty) workspaces field", () => {
    ...
    createFileSystemProject({ rootDirectory: tmpDir, packageManager: "bun" });
    expect(findHintCall("No workspaces declared")).toBeUndefined();
  });
  ...
  test("pnpm: warns when pnpm-workspace.yaml is absent", () => {
    ...
    expect(findHintCall("pnpm-workspace.yaml")).toBeDefined();
  });
