...
describe("getGitAffectedFiles", () => {
  describe("committed range", () => {
    ...
    test("file renamed across paths surfaces the new path with diff reason", async () => {
      // The pacwich resolver maps each changed file to its owning
      // workspace, so a cross-workspace rename should mark the
      // destination workspace affected. We commit add(new) + remove(old)
      // in a single commit so git can detect this as a rename when
      // diff.renames is on (the default).
      await using fixture = await createGitFixture({
        commits: [
          {
            message: "init",
            files: [{ path: "old/file.txt", content: "hello\n" }],
          },
          {
            message: "rename",
            files: [{ path: "new/file.txt", content: "hello\n" }],
            remove: ["old/file.txt"],
          },
        ],
      });
      ...
      const { files } = await getGitAffectedFiles({
        rootDirectory: fixture.projectPath,
        baseRef: fixture.shaForMessage("init"),
        headRef: fixture.shaForMessage("rename"),
        ignoreUncommitted: true,
      });

      const paths = files.map((f) => f.projectFilePath);
      expect(paths).toContain("new/file.txt");
      for (const file of files) {
        expect(file.reasons).toEqual(["diff"]);
      }
    });

    test("rename with diff.renames=false surfaces both endpoints", async () => {
      // Some users explicitly disable git's rename detection. In that
      // mode `--name-only` shows both the old (delete) and new (add)
      // paths, and pacwich must mark both workspaces affected.
      await using fixture = await createGitFixture({
        commits: [
          {
            message: "init",
      ...
      await fixture.runGit(["config", "diff.renames", "false"]);
      ...
      const paths = files.map((f) => f.projectFilePath).sort();
      expect(paths).toEqual(["new/file.txt", "old/file.txt"]);
    });
  });
  ...
  describe("project root resolution", () => {
    ...
    test("throws NoGitRepository when run outside a git repo", async () => {
      const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pacwich-not-git-"));
      try {
        await expect(
          getGitAffectedFiles({
            rootDirectory: dir,
