...
describe("Workspace.externalDependencies", () => {
  test("captures dependencies and devDependencies tagged with their source", () => {
    const { workspaces } = assembleProject({
      adapter,
      rootDirectory: loadFixture("withDependenciesWithExternal"),
    });
    const a = workspaces.find((w) => w.name === "a")!;
    // Source package.jsons pin exact versions so the same lockfile
    // expectations hold across bun and npm; the version string here
    // is what the fixture's package.json declares verbatim.
    expect(a.externalDependencies).toEqual([
      { name: "lodash", version: "4.18.1", source: "dependencies" },
      { name: "typescript", version: "5.9.3", source: "devDependencies" },
    ]);
  });
