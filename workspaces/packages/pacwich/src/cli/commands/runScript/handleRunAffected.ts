...
export type RunAffectedCliOptions = SharedRunScriptCliOptions & {
...
/**
 * Shared implementation behind both `run-affected` (deprecated) and
 * `affected run`.
 */
export const handleRunAffected = async (
  {
    project,
    postTerminatorArgs,
    outputWriters,
...
export const runAffectedDeprecated = handleProjectCommand(
  "runAffected",
  async (
    context,
    positionalScript: string | undefined,
    ...
    logger.warn("DeprecatedRunAffectedCliCommand", {});
    await handleRunAffected(context, positionalScript, options);
  },
);

export const affectedRun = handleProjectCommand(
  "affectedRun",
  handleRunAffected,
);
