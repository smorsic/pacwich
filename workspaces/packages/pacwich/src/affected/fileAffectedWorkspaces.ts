...
/**
 * Whether an affected-dependency edge came from a configured
 * workspace-pattern input ("input") or a real package.json dependency
 * ("package").
 */
export type AffectedDependencyEdgeSource = "input" | "package";

/** One link in the chain explaining how an affected workspace was reached from a changed dependency. */
export interface AffectedDependencyChainEntry {
  /** The workspace at this point in the chain. */
...
// The file-pattern matching primitive lives in `src/inputs` so both this
// engine and `verify` share it. Here it is wrapped to re-attach the
// affected engine's per-file `fileMetadata` (undefined at this stage).
