...
/** Errors thrown during git-mode affected resolution (missing
 * repository, failed `git` invocation, malformed ref). Subclasses of
 * {@link PacwichError}. */
...
/**
 * Why a changed file was flagged in git mode: present in the committed
 * `baseRef..headRef` range ("diff"), or in the working tree as "staged",
...
export interface GetGitAffectedFilesOptions {
   ...
   * `main..feature` or `main...feature`. The two refs are passed as
   * the two endpoints of `git diff`, which is already a two-dot diff.
   */
  baseRef: string;
