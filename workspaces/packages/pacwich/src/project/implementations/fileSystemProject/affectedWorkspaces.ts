...
import type { WorkspaceInputsConfig } from "@pacwich/common";
...
> = BaseAffectedWorkspacesOptions<AcceptsScript> & {
   ...
   * - Globs are expanded via `fs.globSync` against the project root and
...
/**
 * Type guard that narrows {@link DetermineAffectedWorkspacesOptions}
 * to the variant matching `diffSource`. Useful when forwarding a
 * caller-supplied options object into code that needs to read fields
 * specific to one diff source (`diffOptions` for `"git"`,
 * `changedFiles` for `"fileList"`).
 *
 * @example
 * if (isOptionsForDiffSource(options, "git")) {
 *   const ref = options.diffOptions?.baseRef;
 * } else {
...
const expandPatternToFiles = ({
  ...
  if (normalized && GLOB_CHARACTER_REGEX.test(normalized)) {
    return fs
      .globSync(normalized, { cwd: rootDirectory })
      .filter((match) => {
        try {
          return fs.statSync(path.join(rootDirectory, match)).isFile();
        } catch {
          return false;
        }
      })
...
export const determineAffectedWorkspaces = async (
  ...
  if (isOptionsForDiffSource(options, "git")) {
    const baseRef =
      ...
      project.config.project.defaults.affectedBaseRef;
  ...
  const lockfileInChangedFiles = expandedChangedFilePaths.includes(
    adapter.lockfile.projectRelativePath,
  );
