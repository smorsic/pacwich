/**
 * Front-door gate for the web-cli sandbox.
 *
 ...
 * shell operators) and adds git-diff affected resolution, which the sandbox
 * replaces with `--files`.
 *
 ...
 * on the *user* tokens, before the sandbox injects `--cwd`/`--pm`.
 */
export const checkCommandLine = (
  ...
  if (SHELL_OPERATOR.test(rawInput)) {
    return {
      message:
        "Shell operations aren't supported here — the sandbox only passes arguments to `pacwich` (no pipes, redirects, subshells, or background jobs).",
    };
  }
  ...
  if (hasFlag(tokens, ["--cwd", "-d"])) {
    return {
      message:
        "`--cwd` is fixed to the mock project in the web sandbox and can't be changed.",
    };
  }
  ...
  if (hasFlag(tokens, ["--inline", "-i", "--inline-name", "-I"])) {
    return {
      message:
        "Inline scripts (`--inline`) aren't supported in the web sandbox — there's no shell to run them. Run a package.json script instead, e.g. `run build`.",
    };
  }
  ...
  if (commandName(tokens) === "doctor") {
    return {
      message:
        "The `doctor` command isn't available in the web sandbox — it inspects the host environment.",
    };
  }
  ...
  if (hasFlag(tokens, ["--base", "-B", "--head", "-H"])) {
    return {
      message:
        "Git diffs aren't available in the web sandbox — use `--files` instead, e.g. `list-affected --files 'packages/**/*.ts'`.",
    };
  }
