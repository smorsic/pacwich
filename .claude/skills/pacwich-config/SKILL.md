---
name: pacwich-config
description: "Project config (pacwich.project.jsonc) and workspace config (pacwich.workspace.jsonc) schema and options."
when_to_use: "When authoring or editing pacwich.project.* or pacwich.workspace.* config files."
---
...
```jsonc
{
  ...
  // auto-detects from the lockfiles in the project root. The
  // `PACWICH_PACKAGE_MANAGER` env var and the CLI `--pm` flag take
  // precedence over this field. Not nested under "defaults" because it
  // identifies the project, not a per-invocation preference.
...
See also: pacwich-overview, pacwich-concepts, pacwich-cli, pacwich-api.
